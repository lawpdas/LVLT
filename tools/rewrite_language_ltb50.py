...
with open(os.path.join('../RefLTB50', 'check.txt'), 'r') as f:
...
for i, (video_name, im_list, gt_list, _) in enumerate(seqs):
    ...
    for im_i, im_f in enumerate(im_list):
        ...
        index += 1
        if '#' in languages[index]:
            index += 1
        language[im_i] = languages[index].split('|')[-1].strip()
        ...
        gt = np.array(gt_list[im_i])  # [x y w h]
        gt[2:] = gt[2:] + gt[:2] - 1
        gt[0::2] *= sw
        gt[1::2] *= sh
        ...
        img.save(os.path.join(save_path, '{}_{:0>6d}.jpg'.format(video_name, im_i)))

    with open(os.path.join('../RefLTB50', video_name, 'language.txt'), 'w') as f:
        for lang in language:
            f.writelines(lang.strip()+'\n')
